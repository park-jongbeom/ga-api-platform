...
# -*- coding: utf-8 -*-
"""
매핑 테이블 기준으로 코드 완료 + JIRA To Do인 이슈를 완료 처리.
"""
import argparse
import base64
import json
...
def main() -> None:
    parser = argparse.ArgumentParser(description="코드 완료 항목 JIRA 완료 처리")
    ...
    to_complete = [
        ...
        if v.get("needs_completion")
    ]

    print(f"완료 처리 대상: {len(to_complete)}개")
    for jira_key, info in to_complete:
        backlog = info.get("backlog_key", "-")
        title = info.get("backlog_title", "")[:50]
        print(f"  {jira_key} ({backlog}): {title}")
    ...
    success, fail = 0, 0
    for jira_key, info in to_complete:
        if transition_to_done(jira_url, headers, jira_key):
            print(f"  ✓ {jira_key} 완료")
            success += 1
        else:
            print(f"  ✗ {jira_key} 전환 실패")
            fail += 1
        time.sleep(0.4)
